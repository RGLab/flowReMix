...
extern NumericMatrix subsetAssignGibbs(const NumericVector& y,
                                       const NumericVector& prop,
                                       const NumericVector& N,
                                       const NumericMatrix& isingCoefs,
                                       const NumericVector& nullEta,
                                       const NumericVector& altEta,
...
extern NumericMatrix
simRandomEffectCoordinateMH(const NumericVector& y,
                            const NumericVector& N,
                            ...
                            NumericVector MHattempts,
                            NumericVector MHsuccess,
                            const NumericVector& unifVec,
                            const NumericVector& dispersion,
                            const bool betaDispersion,
                            const int keepEach,
...
List CppFlowSstep(const List& subjectData,
                  const int nsamp,
                  const int nSubsets,
                  const int intSampSize,
                  const NumericMatrix& isingCoefs,
                  ...
                  const bool betaDispersion,
                  const double randomAssignProb,
                  const NumericVector& modelprobs,
  ...
  try {
    ...
    if (mixed) {
      ...
      if (markovChainEM || ((CharacterVector) subjectData.names()).cend() ==
          std::find(((CharacterVector) subjectData.names()).cbegin(),
                    ((CharacterVector) subjectData.names()).cend(),
                    "assign")) {
        subjassign = NumericVector(nSubsets);
      } else {
        subjassign = (SEXP) subjectData["assign"];
      }
      assignmentMat = subsetAssignGibbs(y,
                                        prop,
                                        N,
                                        isingCoefs,
                                        nullEta,
