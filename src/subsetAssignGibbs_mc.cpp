...
arma::mat subsetAssignGibbs_mc(const arma::vec y,
  ...
  for (m = 0; m < nsamp ; m++) {
    for (j = 0; j < nSubsets ; j++) {
      ...
      subsetN = N.elem(ipop);
      sigmaHat = sqrt(covariance.at(j, j));
      ...
      for (k = 0; k < 2; k++) {
        ...
        sampNormDens = flowReMix::dnorm4(vsample, muHat,
                                         sigmaHat * mcoef,
                                         true);
      ...
      densityRatio = integratedDensities[0] /
          integratedDensities[1] * (1.0 - priorProb) /
          priorProb;
