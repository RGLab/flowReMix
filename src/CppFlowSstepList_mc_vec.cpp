...
#pragma omp for
  ...
  for (int subject = 0; subject < nsubjects; ++subject) {
      ...
      for (sample = 0; sample < nsamp_floor; sample++) {
        ...
        if ((sample % keepEach) == 0) {
          for (int i = 0; i < clusterassignments.col(subject).n_elem; i ++) {
            assignmentMats(assignNum, i, subject) =
                clusterassignments(i, subject);
          }
          assignNum++;
        }
      }
      ...
      if (sampleRandom) {
        ...
        randomeffectMats.slice(subject) =
...
// Iterate over columns and assign to lists or
// just return a list of the three matrices..
