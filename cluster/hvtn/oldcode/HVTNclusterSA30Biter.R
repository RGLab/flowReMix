...
# subsetDat <- subset(subsetDat, stimGroup == "env")
subsetDat$subset <- factor(as.character(subsetDat$subset))

configurations <- expand.grid(niter = c(30),
                              seed = 1,
                              prior = c(0),
                              method = c("SA"))
...
control <- flowReMix_control(updateLag = lag, nsamp = 50,
                             ...
                             maxDispersion = 1000, minDispersion = 10^6,
...
file <- paste("results/hvtn_10_niter", niter, "npost", npost, "seed", seed, "prior", prior, method, ".rds", sep = "")
