...
stimdat$stimVisit[grepl(".5", stimdat$stimVisit, fixed = TRUE)] <- "ctrl"
stimdat$stimVisit <- factor(stimdat$stimVisit)
fit <- flowReMix(cbind(count, parentcount - count) ~ stim + stimVisit,
                subject_id = ptid,
...
fit$data <- NULL
filename <- paste("results/HVTN105temp_noVisitInt_A",
