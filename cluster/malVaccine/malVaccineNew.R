...
ncores <- 2
mode <- "M0M3childrenInteraction"
mode <- "M3children"
mode <- "M0M3stimTreatment"
mode <- "M3infantsChildren"
mode <- "M0M3stimInfants"
mode <- "M0M3interactionInfants"

# Loading Data ---------
tempdat <- readRDS(file = "data/mal067_totals_A.rds")
tempdat$stim <- factor(tempdat$stim)
if(mode == "M0M3childrenInteraction") {
...
} else if(mode == "M0M3interactionInfants") {
  tempdat$treatment <- factor(with(tempdat, visitno == "M3" & stim != "ctrl"))
...
#               "niter", niter,
#               ".rds", sep ="")
file <- paste("results/malVaccine_totals_",
              mode, "_",
              "disp", disp,
              "seed", seed,
              "npost", npost,
